...
def main(
    ...
    pretrained_path: str = "checkpoints/lit-llama/7B/consolidated.00.pth",
    tokenizer_path: str = "checkpoints/lit-llama/7B/tokenizer.model",
    ...
    out_dir = f"out/subnet/alpaca/sub-{subnet_mode}_sparsity_attn-{sparsity_attn}_sparsity_mlp-{sparsity_mlp}_ratio-{ratio}"
    os.makedirs(out_dir, exist_ok=True)

    global logger
    logger = logging.getLogger(f'sub-{subnet_mode}_sparsity_attn-{sparsity_attn}_sparsity_mlp-{sparsity_mlp}_ratio-{ratio}')
    ...
    with fabric.device, fabric.init_module():
    ...
    # print("Model structure after setup_module:")
    # print(model)
        ...
        if not is_accumulating:
            ...
            for name, old_param in params_to_check.items():
                ...
                if not torch.equal(old_param, new_param):
                    logger.info(f"Parameter {name} has been updated!")
