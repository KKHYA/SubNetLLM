...
class CausalSelfAttention_sub(nn.Module):
    def __init__(self, config: LLaMAConfig) -> None:
        ...
        # key, query, value projections for all heads, but in a batch
        self.c_attn = SupermaskLinear(config.n_embd, 3 * config.n_embd, bias=False, sparsity=config.sparsity_attn)
        # output projection
        self.c_proj = SupermaskLinear(config.n_embd, config.n_embd, bias=False, sparsity=config.sparsity_attn)
...
class MLP_sub(nn.Module):
    def __init__(self, config: LLaMAConfig) -> None:
        ...
        self.c_fc1 = SupermaskLinear(config.n_embd, n_hidden, bias=False, sparsity=config.sparsity_mlp)
        self.c_fc2 = SupermaskLinear(config.n_embd, n_hidden, bias=False, sparsity=config.sparsity_mlp)
        self.c_proj = SupermaskLinear(n_hidden, config.n_embd, bias=False, sparsity=config.sparsity_mlp)
